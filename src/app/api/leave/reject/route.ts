...
export async function POST(request: Request) {
  ...
  try {
    ...
    const record = await leaveService.reject(user, body);
    return NextResponse.json(record);
  } catch (error) {
    console.error("[leave/reject POST]", error);
    const message = error instanceof Error ? error.message : "Leave rejection failed";
