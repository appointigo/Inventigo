...
export async function POST(request: Request) {
  ...
  if (!verifyEmbeddedSignupState(expected ? decodeURIComponent(expected) : undefined, parsed.data.state, user.id, user.orgId)) return clearSignupState(NextResponse.json({ error: "Invalid signup state", code: "EMBEDDED_SIGNUP_INVALID_STATE" }, { status: 403 }));
  try {
    ...
    return clearSignupState(NextResponse.json({ error: "WhatsApp connection could not be completed", code }, { status: code === "META_AUTH_FAILED" ? 401 : 502 }));
  }
}
