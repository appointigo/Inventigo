  ...
  if (process.env.RESEND_API_KEY) {
    try {
        ...
        body: JSON.stringify({
          ...
          html: `
            <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
              ...
              <p style="color:#999;font-size:12px">If you didn't create a Stockiva account, you can ignore this email.</p>
            </div>
          `,
      ...
      if (!res.ok) {
        ...
        console.error(`[register] Resend API error for ${email}:`, err);
        if (process.env.NODE_ENV !== "production") {
          console.warn("[register] Resend email failed in development, falling back to console output.");
      ...
      console.error(`[register] Resend request failed for ${email}:`, error);
      if (process.env.NODE_ENV !== "production") {
        console.warn("[register] Resend email request failed in development, falling back to console output.");
        console.log(`\n[Stockiva OTP] Email: ${email} | Code: ${code}\n`);
        return;
      }
      throw new Error("Failed to send verification email");
