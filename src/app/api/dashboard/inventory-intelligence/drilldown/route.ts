...
export const GET = async (request: Request) => {
  ...
  try {
    ...
    if (!categoryId) throw new AnalyticsPeriodError("A category is required");
    ...
    console.error("[inventory-intelligence drilldown GET]", error);
    return NextResponse.json({ error: "Unable to load category detail" }, { status: 500 });
  }
};
