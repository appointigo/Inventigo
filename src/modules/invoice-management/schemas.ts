...
const invoiceManagementSettingsShape = {
  ...
  exchangePolicyText: optionalPolicyText.optional(),
  returnPolicyText: optionalPolicyText,
  thankYouMessage: z.string().trim().max(500).nullable(),
  storeSubtitle: z.string().trim().max(200).nullable().optional(),
  footerNote: z.string().trim().max(500).nullable().optional(),
  signatureText: z.string().trim().max(200).nullable().optional(),
  qrHelperText: z.string().trim().max(300).nullable().optional(),
