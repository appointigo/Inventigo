...
export const testMessageSchema = z.object({
  ...
  recipient: z
    .string()
    .trim()
    .regex(/^\+?[1-9]\d{7,14}$/, "Enter a valid international WhatsApp number"),
...
export const messageActivityQuerySchema = z.object({
  ...
  purpose: z
    .enum(["INVOICE", "PAYMENT", "MARKETING", "ORDER", "SUPPORT", "OTP", "OTHER"])
    .optional(),
...
export const conversationQuerySchema = z.object({
  page: z.coerce.number().int().positive().default(1),
  pageSize: z.coerce.number().int().min(1).max(100).default(20),
  routingStatus: z.enum(["RESOLVED", "UNRESOLVED"]).optional(),
  search: z.string().trim().max(100).optional(),
});
