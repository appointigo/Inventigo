...
export class WhatsAppTestMessageService {
  ...
  async options(organizationId: string) {
    const [stores, templates] = await Promise.all([
      this.prisma.store.findMany({ where: { orgId: organizationId, isActive: true, whatsappSenders: { some: { isActive: true, phoneNumber: { status: "ACTIVE", waba: { status: "ACTIVE", integration: { organizationId, status: "CONNECTED", credentialRef: { not: null } } } } } } }, select: { id: true, name: true, code: true, whatsappSenders: { where: { isActive: true, phoneNumber: { status: "ACTIVE", waba: { status: "ACTIVE", integration: { organizationId, status: "CONNECTED", credentialRef: { not: null } } } } }, select: { purpose: true, isDefault: true, priority: true, phoneNumber: { select: { displayPhoneNumber: true, verifiedName: true, status: true } } }, orderBy: [{ priority: "asc" }] } }, orderBy: { name: "asc" } }),
      this.prisma.whatsAppTemplateInstance.findMany({ where: { status: "APPROVED", waba: { status: "ACTIVE", integration: { organizationId, status: "CONNECTED", credentialRef: { not: null } } }, definition: { isActive: true } }, select: { id: true, metaTemplateName: true, definition: { select: { key: true, version: true, language: true, purpose: true, body: true, variables: true } }, waba: { select: { id: true, businessName: true } } }, orderBy: { metaTemplateName: "asc" } }),
    ]);
