...
export default function WhatsAppSetupPage() {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(response.status === 403 ? "Only an organization owner or admin can connect WhatsApp." : "WhatsApp setup is currently unavailable.");
      const session = await response.json() as SessionResponse; const sdk = await loadMetaSdk(session.appId, session.graphApiVersion);
  ...
  return <WhatsAppShell status={<WhatsAppStatusBadge state={currentState} />}>
    {loading ? <Surface><WhatsAppLoadingState /></Surface> : error ? <Surface><WhatsAppErrorState onRetry={() => void loadStatus()} /></Surface> : phase === "syncing" ? <Surface><Result icon={<Spin size="large" />} title="Setting up WhatsApp" subTitle="Stockiva is securely exchanging authorization and syncing your business accounts and phone numbers." extra={<WhatsAppSetupProgress current={2} />} /></Surface> : isDisconnected ? <>
      {phase === "cancelled" && <Alert type="info" showIcon closable message="Meta setup was cancelled" description="Nothing was connected. You can continue again whenever you’re ready." onClose={() => setPhase("idle")} />}
      {phase === "failed" && <Alert type="error" showIcon message="WhatsApp could not be connected" description={flowError} action={<Button onClick={() => void connect()}>Retry</Button>} />}
      ...
      {phase === "failed" && <Alert type="error" showIcon message="WhatsApp sync failed" description={flowError} />}
