...
export function buildInvoiceDocumentHtml(input: InvoiceDocumentInput) {
  ...
  const policy = model.termsText || model.returnPolicyText ? `<div class="policy">${model.termsText ? `<section><strong>Terms &amp; Conditions</strong><p>${escapeHtml(model.termsText)}</p></section>` : ""}${model.returnPolicyText ? `<section><strong>Exchange / Return Policy</strong><p>${escapeHtml(model.returnPolicyText)}</p></section>` : ""}</div>` : "";
  const closing = [model.qrHelperText, model.signatureText, model.footerNote].filter(Boolean).map(value => `<p>${escapeHtml(value)}</p>`).join("");
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(model.title)} ${escapeHtml(model.reference)}</title><style>
    ...
    .policy{margin-top:24px;padding-top:12px;border-top:1px solid #dfe3ea;color:#566074;font-size:10px}.policy section{margin:0 0 10px}.policy p{white-space:pre-wrap;margin:4px 0}.PREMIUM{color:#172554}.PREMIUM .header{text-align:left;background:#f8f5ec;border-color:#c59a3d;padding:18px}.PREMIUM th{background:#172554;color:#fff}.COMPACT{font-size:10px}.COMPACT .header{text-align:left;padding-bottom:8px;margin-bottom:10px}.COMPACT th,.COMPACT td{padding:5px}.COMPACT section{margin-top:10px}.COMPACT .totals{margin-top:10px}
  </style></head><body class="${model.designKey}"><div class="header"><h1>${escapeHtml(model.merchant.name)}</h1>${model.merchant.subtitle ? `<p>${escapeHtml(model.merchant.subtitle)}</p>` : ""}<p>${escapeHtml(model.title)}</p></div><div class="meta"><div><strong>Reference:</strong> ${escapeHtml(model.reference)}<br><strong>Date:</strong> ${escapeHtml(model.issuedAt)}<br>${model.originalInvoice ? `<strong>Original invoice:</strong> ${escapeHtml(model.originalInvoice)}<br>` : ""}<strong>Payment:</strong> ${escapeHtml(model.paymentMethod)}</div><div>${model.customerName ? `<strong>Customer:</strong> ${escapeHtml(model.customerName)}<br>` : ""}${model.customerPhone ? `<strong>Phone:</strong> ${escapeHtml(model.customerPhone)}<br>` : ""}<strong>Status:</strong> ${escapeHtml(model.status)}</div></div>${sections}<div class="totals">${totals}</div>${policy}<div class="footer">${escapeHtml(model.thankYouMessage || "Thank you for shopping with us.")}${closing}</div></body></html>`;
}
