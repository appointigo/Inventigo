...
type ReturnTransactionRelationItem = {
  ...
  returnedLineAmount?: Prisma.Decimal | number | string | null;
    ...
    relationalItems.length > 0
      ? relationalItems
          ...
          .map((item) => ({
            ...
            total: Number(
              item.returnedLineAmount ??
                round2(Number(item.returnedUnitPrice ?? 0) * Number(item.returnedQuantity ?? 0))
            ),
  ...
  return {
    ...
    items: (s.items ?? []).map(
      (i: any): SaleItem => ({
        ...
        originalUnitPrice: i.originalUnitPrice != null ? Number(i.originalUnitPrice) : undefined,
        itemDiscountType: i.itemDiscountType ?? undefined,
        itemDiscountValue: i.itemDiscountValue != null ? Number(i.itemDiscountValue) : undefined,
        netLineAmount: i.netLineAmount != null ? Number(i.netLineAmount) : undefined,
        quantity: i.quantity,
...
export const billingService = {
  async createSale(
    ...
    for (const item of input.items) {
      if (
        ...
        throw new Error("Invalid sale item quantity or variant");
      const key = `${item.productId}:${item.sizeId}`;
      if (keys.has(key)) throw new Error("Invalid duplicate sale variant");
      keys.add(key);
      validateAmount(item.unitPrice, "unit price");
    }
    if (!(await prisma.store.findFirst({ where: { id: storeId, orgId, isActive: true } })))
      throw new Error("Invalid store");
    const products = await prisma.product.findMany({
      where: { id: { in: input.items.map((item) => item.productId) }, orgId, isActive: true },
      select: { id: true, mrp: true, basePrice: true, costPrice: true },
    });
    const productMap = new Map(products.map((product) => [product.id, product]));
    const sourceItems = input.items.map((item) => {
      const product = productMap.get(item.productId);
      if (!product) throw new Error("Product not found or inactive");
      const originalUnitPrice = Number(product.basePrice);
      // Legacy callers already send an editable unitPrice. New callers send intent;
      // recompute that intent against the server's current normal selling price.
      const adjustment =
        item.itemDiscountType == null
          ? { itemDiscountType: "PRICE" as const, itemDiscountValue: item.unitPrice }
          : { itemDiscountType: item.itemDiscountType, itemDiscountValue: item.itemDiscountValue };
      return {
        ...adjustment,
        productId: item.productId,
        quantity: item.quantity,
        originalUnitPrice,
        sellingPrice: resolveItemPrice(originalUnitPrice, adjustment),
        mrp: Number(product.mrp),
        costPrice: Number(product.costPrice),
        eligibleForDiscount: true,
      };
    });
    const subtotal = sourceItems.reduce((sum, item) => sum + item.sellingPrice * item.quantity, 0);
    if (input.amountPaid != null) validateAmount(input.amountPaid, "amount paid");
    const transactionDate = input.transactionDate ? new Date(input.transactionDate) : new Date();
    if (!Number.isFinite(transactionDate.getTime())) throw new Error("Invalid transaction date");
    ...
    if (input.promoCodeId) {
      ...
      promoPercent = Number(promo.discountPct);
      discountAmount = round2((subtotal * promoPercent) / 100);
      resolvedPromoCodeId = promo.id;
    }

    const pricing = allocatePricingSnapshots(sourceItems, {
      discountType:
        promoPercent != null
          ? "PERCENTAGE"
          : (input.discountType ?? (input.discountPercent != null ? "PERCENTAGE" : "FLAT")),
      discountPercent: promoPercent ?? input.discountPercent ?? 0,
      discountAmount,
              ...
              items: {
                create: input.items.map((it, index) => {
                  ...
                  return {
                    ...
                    total: round2(snapshot.sellingPrice * it.quantity),
                    originalUnitPrice: snapshot.originalUnitPrice,
                    itemDiscountType: snapshot.itemDiscountType,
                    itemDiscountValue: snapshot.itemDiscountValue,
                    netLineAmount: snapshot.netLineAmount,
    ...
    const rtRows = await Promise.all(
      returnTxns.map(async (r: any) => {
        ...
        if (Array.isArray(r.items) && r.items.length > 0) {
          for (const item of r.items) {
            if (item.returnedProductId) {
              items.push({
                ...
                sizeLabel: item.returnedSize?.label ?? item.sizeLabel ?? undefined,
                quantity: Number(item.returnedQuantity ?? 0),
                unitPrice: Number(item.returnedUnitPrice ?? 0),
                total: Number(
                  item.returnedLineAmount ??
                    round2(Number(item.returnedUnitPrice ?? 0) * Number(item.returnedQuantity ?? 0))
                ),
            ...
            if (item.newProductId) {
              items.push({
                ...
                sizeLabel: item.newSize?.label ?? item.sizeLabel ?? undefined,
                quantity: Number(item.newQuantity ?? 0),
                unitPrice: Number(item.newUnitPrice ?? 0),
                total: round2(Number(item.newUnitPrice ?? 0) * Number(item.newQuantity ?? 0)),
              });
            }
          }
          ...
          for (const item of legacyReturned) {
            items.push({
              type: "RETURNED",
              ...
              quantity: item.quantity,
              unitPrice: round2(item.total / Math.max(1, item.quantity)),
              total: item.total,
            });
          }
          for (const item of legacyExchanged) {
            items.push({
              type: "NEW",
              productName: item.productName,
              sku: item.sku,
        ...
        return {
          ...
          payments: (paymentsInfo.payments ?? []).map((payment: any) => ({
            ...payment,
            amount: Number(payment.amount ?? 0),
            businessDate:
              payment.businessDate instanceof Date
                ? payment.businessDate.toISOString()
  ...
  async createReturnTransaction(
    ...
    const previousSale = await billingService.getSaleById(orgId, saleId);
    const previousQuantities = new Map<string, number>();
    for (const transaction of previousSale?.returnTransactions ?? []) {
      for (const item of transaction.returnedItems) {
        const key = `${item.productId}:${item.sizeId}`;
        previousQuantities.set(key, (previousQuantities.get(key) ?? 0) + item.quantity);
      }
    }
    const requestKeys = new Set<string>();
    const returnedLineItems = input.returnedItems.map((item) => {
      const key = `${item.productId}:${item.sizeId}`;
      if (requestKeys.has(key)) throw new Error("Invalid returned quantity: duplicate variant");
      requestKeys.add(key);
      const historicalSaleItem = saleItemsByKey.get(`${item.productId}:${item.sizeId}`);
      const historicalUnitAmount = historicalSaleItem
        ? getHistoricalUnitAmount(historicalSaleItem)
        : round2(Number(item.total) / Math.max(1, item.quantity));

      return {
        ...item,
        historicalUnitAmount,
        total: historicalSaleItem
          ? getHistoricalReturnAmount(
              historicalSaleItem,
              item.quantity,
              previousQuantities.get(key) ?? 0
            )
          : 0,
      };
    });

    const exchangeProducts = await prisma.product.findMany({
      where: {
        id: { in: (input.exchangedItems ?? []).map((item) => item.productId) },
        orgId,
        isActive: true,
      },
      select: { id: true },
    ...
    const exchangedLineItems = (input.exchangedItems ?? []).map((item) => {
      validateAmount(item.total, "exchange price");
      if (!exchangeProducts.some((product) => product.id === item.productId))
        throw new Error("Invalid exchange product");
      return { ...item, total: round2(item.total) };
    });
    ...
    const returnedQty =
      input.returnedItems.reduce((sum, item) => sum + item.quantity, 0) +
      [...previousQuantities.values()].reduce((sum, qty) => sum + qty, 0);
    for (const item of input.returnedItems) {
      ...
      if (
        !Number.isSafeInteger(item.quantity) ||
        item.quantity <= 0 ||
        item.quantity + (previousQuantities.get(key) ?? 0) > existingSaleItem.quantity
      ) {
    ...
    const exchangePricing = allocatePricingSnapshots(
      exchangedLineItems.map((item) => ({
        productId: item.productId,
        quantity: item.quantity,
        mrp: item.total / item.quantity,
        sellingPrice: item.total / item.quantity,
      })),
      { discountType, discountPercent, discountAmount: input.discountAmount ?? 0, taxRate }
    );
    const discountAmount = exchangePricing.discountAmount;
